...
public record ArrayStringRefClassifier(String variable, int size, boolean zeroIsNull) implements Classifier {
    ...
    public ClassifierResult find(String thisName, ByteView data, Template template, ParseObjects objects) throws ParseException {
        ...
        if(index < 0 || index >= innerObjects.size()) throw new ParseException(view, objects, "Error in Union %s: index %d not found in array %s".formatted(thisName, index, variable));
        Data structNameObject = innerObjects.get(index).data();
        if(!(structNameObject instanceof StringData(String val))) throw new ParseException(view, objects, "Error in Union %s: element found in array %s at index %d is not a string: %s".formatted(thisName, variable, index, structNameObject.type()));
